...
void yeti_model_draw(yeti_t* yeti, entity_t* entity)
{
  ...
  matrix_rotate_object(om,
    ...
    (entity->z + entity->FixZ ) - yeti->camera->z,    //fix z in some models
  ...
  if (entity->rz < yeti->visual.threshold_perspective && entity->animation.speed)
  {
    animation_get_frames(&entity->animation, &i, &j);
    {
      ...
      for (i = 0; i < model[2]; i++, v1 += 3, v2 += 3)
      {
        ...
        yeti->verts[i].x = (FIXDOT(wm[0] , v)  * entity->ScaleModel ) + wm[0][3]  ;  //escalar modelo
        yeti->verts[i].y = (FIXDOT(wm[1] , v)  * entity->ScaleModel ) + wm[1][3]  ;
        yeti->verts[i].z = (FIXDOT(wm[2] , v)  * entity->ScaleModel ) + wm[2][3]  ;
  ...
  else  //lejos camara
  {
    ...
    {
      yeti->verts[i].x = (FIXDOT(wm[0], v1)  * entity->ScaleModel) + wm[0][3];  //escalar modelo
      yeti->verts[i].y = (FIXDOT(wm[1], v1)  * entity->ScaleModel) + wm[1][3];
      yeti->verts[i].z = (FIXDOT(wm[2], v1)  * entity->ScaleModel) + wm[2][3];
...
extern void bala_tick(yeti_t* yeti, entity_t* e);
#ifdef  COMPILANDO_BENNU
 #define BENNU_CHEQUEO_BALAS   &&  !((e1->ontick == bala_tick) &&  (e2->ontick == bala_tick ))
#else
 #define BENNU_CHEQUEO_BALAS
#endif
...
#define IS_COLLISION (e1->z<(b=cell_bot(cell,e1))||e1->z>(t=cell_top(cell,e1))||((t-b)<192))
     if (!e1->nocollisionwall)//colision con paredes
     {
...
#undef IS_COLLISION
      ...
      for (j = i + 1; j < yeti->entities.length; j++)
      {
        ...
        if (e2->radius    BENNU_CHEQUEO_BALAS  )
        {
